...
13. Deploy a function with the Nuclio CLI (nuctl) (for local repositories it is `localhost:5000`, if we use docker hub the url is `docker.io/<username>` with your dockerhub username) `nuctl deploy helloworld -n nuclio -p https://raw.githubusercontent.com/nuclio/nuclio/master/hack/examples/golang/helloworld/helloworld.go --registry <URL>`
