...
13. Deploy a function with the Nuclio CLI (nuctl) (for local repositories thr URL is `--registry $(minikube ip):5000 --run-registry localhost:5000`, if we use docker hub the URL is `--registry docker.io/<username>` with your dockerhub username) `sudo nuctl deploy helloworld -n nuclio -p https://raw.githubusercontent.com/nuclio/nuclio/master/hack/examples/golang/helloworld/helloworld.go <URL>`
